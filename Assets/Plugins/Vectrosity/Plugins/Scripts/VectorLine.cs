...
public partial class VectorLine {
	...
	public VectorLine (string name, List<Vector3> points, float width) {
		m_points3 = points;
		SetupLine (name, null, width, LineType.Discrete, Joins.None, false, null);
	}
	public VectorLine (string name, List<Vector3> points, Texture texture, float width) {
		m_points3 = points;
		SetupLine (name, texture, width, LineType.Discrete, Joins.None, false, null);
		}
	
	public VectorLine (string name, List<Vector3> points, float width, LineType lineType) {
		m_points3 = points;
		SetupLine (name, null, width, lineType, Joins.None, false, null); 
	}
	public VectorLine (string name, List<Vector3> points, Texture texture, float width, LineType lineType) {
		m_points3 = points;
		SetupLine (name, texture, width, lineType, Joins.None, false, null);
	}
	public VectorLine (string name, List<Vector3> points, Texture texture, float width, LineType lineType, Transform parent) {
			m_points3 = points;
			SetupLine (name, texture, width, lineType, Joins.None, false, parent);
		}
	
	public VectorLine (string name, List<Vector3> points, float width, LineType lineType, Joins joins) {
		m_points3 = points;
		SetupLine (name, null, width, lineType, joins, false, null);
		}
	public VectorLine (string name, List<Vector3> points, Texture texture, float width, LineType lineType, Joins joins) {
		m_points3 = points;
		SetupLine (name, texture, width, lineType, joins, false, null);
		}
	
	// Vector2 constructors
	public VectorLine (string name, List<Vector2> points, float width) {
		m_points2 = points;
		SetupLine (name, null, width, LineType.Discrete, Joins.None, true, null);
		}
	public VectorLine (string name, List<Vector2> points, Texture texture, float width) {
		m_points2 = points;
		SetupLine (name, texture, width, LineType.Discrete, Joins.None, true, null);
		}
	
	public VectorLine (string name, List<Vector2> points, float width, LineType lineType) {
		m_points2 = points;
		SetupLine (name, null, width, lineType, Joins.None, true, null);
		}
	public VectorLine (string name, List<Vector2> points, Texture texture, float width, LineType lineType) {
		m_points2 = points;
		SetupLine (name, texture, width, lineType, Joins.None, true, null);
		}
	
	public VectorLine (string name, List<Vector2> points, float width, LineType lineType, Joins joins) {
		m_points2 = points;
		SetupLine (name, null, width, lineType, joins, true, null);
		}
	public VectorLine (string name, List<Vector2> points, Texture texture, float width, LineType lineType, Joins joins) {
		m_points2 = points;
		SetupLine (name, texture, width, lineType, joins, true, null);
		}
	
	protected void SetupLine (string lineName, Texture texture, float width, LineType lineType, Joins joins, bool use2D, Transform parent) {
		m_is2D = use2D;
		m_lineType = lineType;
	...
	private void SetupCanvasState (CanvasState wantedState) {
		...
		if (m_go == null) return;
			...
			m_go.transform.SetParent (m_parent);
	...
	public static void SetEndCap (string name, EndCap capType, float offsetFront, float offsetBack, float scaleFront, float scaleBack, params Texture2D[] textures) {
		...
		if (capDictionary.ContainsKey (name) && capType != EndCap.None) {
			//Debug.LogWarning ("VectorLine.SetEndCap: end cap \"" + name + "\" has already been set up");
			return;
		}
