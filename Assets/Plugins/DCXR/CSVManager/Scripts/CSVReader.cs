...
namespace DCXR
{
    ...
    public class CSVReader
    {
        ...
        public List<string> GetCol(string colName)
        {
            ...
            if (!indexTable.ContainsKey(colName))
            {
                Debug.LogError($"Cannot find {colName} in dataTable");
                return res;
            }
            ...
            foreach (var r in contextList)
            {
               ...
               if( r.Count <= indexTable[colName])
                {
                    //Debug.LogWarning("fail to load this row data");
                    continue;
                }
                else
                {
        ...
        public string GetCell(int row, string colName)
        {
            if (!indexTable.ContainsKey(colName))
            {
                Debug.LogError($"table does not contain colname{colName}");
            }
            //Debug.Log($"row {row}, {colName}");
